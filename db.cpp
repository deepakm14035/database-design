...
int sem_select(token_list *t_list)
{
	...
	token_list *cur;
	tpd_entry *tab_entry = NULL;
	tpd_entry **tab_entries = NULL;
	char*** recordsList=NULL;
	int tableCount=0;
	bool column_done = false;
	int cur_id = 0;
	cd_entry  *col_entry = NULL;
	...
	else
	{
		...
		else
		{
			...
			{
				tab_entries=(tpd_entry**)calloc(10,sizeof(tpd_entry*));
				recordsList=(char***)calloc(10,sizeof(char**));
				tab_entries[tableCount]=tab_entry;
				...
				if(columnObjUpdated<0){
					...
					int* rowLens=(int*)calloc(1,sizeof(int));
					rowLens[0]=tfh->num_records;
					...
					if(cur->tok_value == K_NATURAL){
						...
						if(cur->tok_value == K_JOIN){
							...
							if(tab_entry_new_table!=NULL){
								tab_entries[tableCount]=tab_entry_new_table;
								char** newRowData=getTableData(tab_entry_new_table,tfh_new);
								rowLens[tableCount]=tfh_new->num_records;
								recordsList[tableCount++]=newRowData;
					...
					if(cur->tok_value == K_WHERE)
					{
						...
						int conditionCount = parseWhereClause(cur, tab_entries, tableCount, conditionList);
						if(conditionCount<0){
							rc = conditionCount;
							return rc;
						}
						...
						rowsToPrint=filterRows(recordsList, tab_entries, tableCount, conditionList, conditionCount, rowLens, tfh->record_size);
						//printf("%s\n", cur->tok_string);
					...
					if(tableCount>1){
						...
						updatedRows=(char**)calloc(tfh->num_records*tfh_new->num_records,sizeof(char*));
						int columnObjUpdated = filterColumns(columnsInSelect,columnCountInSelect,tab_entry_new_table);
						rowCount=getJoinedData(tab_entry, tab_entry_new_table, records,recordsList[1], tfh, tfh_new, columnsInSelect, columnCountInSelect, updatedRows, rowsToPrint);
						//printf("new rows-%d\n", rowCount);
					}
					...
					if(cur->tok_value == K_GROUP){
						...
						if(cur->tok_value == K_BY){
							...
							for(int i=0;i<columnCountInSelect;i++){
								if(strcasecmp(columnsInSelect[i]->columnName,cur->tok_string)!=0 && columnsInSelect[i]->functionType==NONE){
									cur->tok_value = INVALID;
							...
							for(int i=0;i<tfh->num_records;i++){
								...
								if(rowsToPrint[0]!=NULL) rowsToPrint[0][i] = rows[i]->shouldPrint;
								//printf("group no - %d\n",groupNo);
							...
							handleGroupBy(updatedRows, records, tab_entry, columnsInSelect, columnCountInSelect, rowsToPrint, groupNos, rowCount, newRowLen, tfh);
							cur=cur->next;
						}else{
							rc=INVALID_SYNTAX;
							cur->tok_value = INVALID;
		...
		if(columnsInSelect[i]->functionType!=NONE){
			...
			for(int rowNo=0;rowNo<tfh->num_records;rowNo++){
				...
				for(j=rowNo+1;j<tfh->num_records && groupNos[j]==groupNo;j++){
					...
					if(cellLen==0||(rowsToPrint[0]!=NULL && !rowsToPrint[0][j])) continue;
...
int getJoinedData(tpd_entry* tab_entry1, tpd_entry* tab_entry2, char** records1, char** records2, table_file_header* tfh1, table_file_header* tfh2, select_attribute** columnsInSelect, int columnCount, char** outputRows, bool** rowsToPrint){
	...
	for(int row1=0;row1<tfh1->num_records;row1++){
		//printf("[row1 %d] T or F %d row count %d\n",row1,rowsToPrint[0]!=NULL && !rowsToPrint[0][row1], tfh2->num_records);
		if(rowsToPrint[0]!=NULL && !rowsToPrint[0][row1]) continue;
		for(int row2=0;row2<tfh2->num_records;row2++){
			//printf("[row2 %d] T or F %d\n",row2,rowsToPrint[1]!=NULL && !rowsToPrint[1][row2]);
			if(rowsToPrint[1]!=NULL && !rowsToPrint[1][row2]) continue;
			bool allMatch=true;
...
int sem_delete_from(token_list *t_list)
{
	...
	else
	{
		...
		else
		{
			FILE *fhandle = NULL;
			table_file_header* tfh = (table_file_header*)calloc(1, sizeof(table_file_header));
			recordList=(char***)calloc(10,sizeof(char**));
			tab_entries=(tpd_entry**)calloc(10,sizeof(tpd_entry*));
			tab_entries[tableNo]=tab_entry;
			//tab_entries=(tpd_entry**)calloc(10,sizeof(tpd_entry));
			char* tname=(char*)calloc(1, MAX_TOK_LEN);
			memcpy(tname, tab_entry->table_name, MAX_TOK_LEN);
			strcat(tname, ".tab");
			...
			else
			{
				...
				fflush(fhandle);
				fclose(fhandle);
				recordList[tableNo++]=records;
				cur = cur->next;
				if(cur->tok_value == K_WHERE)
				{
					cur = cur->next;
					condition* conditionList = (condition*)calloc(1, 100);
					int conditionCount = parseWhereClause(cur, tab_entries, tableNo, conditionList);
					if(conditionCount<0){
						rc = conditionCount;
						return rc;
					}
					int* rowLens=(int*)calloc(1,sizeof(int));
					rowLens[0]=tfh->num_records;
					rowsToDelete=filterRows(recordList, tab_entries, tableNo, conditionList, conditionCount, rowLens, tfh->record_size)[0];
				}
				col_entry = (cd_entry*)((char*)tab_entry + tab_entry->cd_offset);
				int cellLen=0;
	...
	else
	{
		...
		else
		{
			...
			if(cur->tok_value == K_WHERE)
			{
				...
				int* rowLens=(int*)calloc(1,sizeof(int));
				rowLens[0]=tfh->num_records;
				rowsToUpdate = filterRows(recordList, tab_entries, tableNo, conditionList, conditionCount, rowLens, tfh->record_size)[0];
...
bool** filterRows(char*** recordsList, tpd_entry **tab_entries, int tableCount, condition* conditionList, int conditionCount, int* rowCounts, int rowLen){
	bool** filters=(bool**)calloc(1, sizeof(bool*));//rowCount
	tpd_entry *tab_entry;
	//printf("[filterRows] conditioncount %d, rowCount %d\n",conditionCount, rowCount);
	for(int t=0;t<tableCount;t++){
		filters[t]=(bool*)calloc(rowCounts[t], sizeof(char));
		for(int i=0;i<rowCounts[t];i++){
			filters[t][i]=true;
		}
	}
	for(int i=0;i<conditionCount;i++){
		//printf("\n[filterRows] condition: %d, %s %d\n",conditionList[i].colNo,conditionList[i].data, conditionList[i].keywordLen);
		tab_entry=tab_entries[conditionList[i].tableNo];
		int t=conditionList[i].tableNo;
		for(int r=0;r<rowCounts[t];r++){
			if(filters[t][r] || (i>0 && conditionList[i-1].nextBinaryOperator==K_OR)){
				cd_entry  *col_entry = NULL;
				col_entry = (cd_entry*)((char*)tab_entry + tab_entry->cd_offset);
				col_entry+=conditionList[i].colNo;
				//col_entry->col_len;
				//printf("getting column index %d table no %d\n",conditionList[i].colNo, t);
				int offset=getRowOffset(tab_entry, conditionList[i].colNo);
				//position at offset stores the length of the cell 
				//printf("getting offset %d row %d\n",offset, r);
				bool isCellNull = *(recordsList[t][r]+offset)==0;
				offset++;
				...
				if(!isCellNull){
					...
					copyBytes(cell, recordsList[t][r]+offset, col_entry->col_len);
				}
				...
				if(i>0){
					if(conditionList[i-1].nextBinaryOperator==K_AND){
						filters[t][r]=filters[t][r] && result;
					}
...
int parseWhereClause(token_list* cur, tpd_entry** tab_entries, int tableCount, condition* conditionList){
	int conditionNo=0;
	cd_entry  *col_entry = NULL;
	//col_entry = (cd_entry*)((char*)tab_entry + tab_entry->cd_offset);
	while(cur->next!=NULL){
		...
		if(cur->tok_value==T_CHAR || cur->tok_value==IDENT){
			bool contains=false;
			for(int t=0;t<tableCount;t++){
				col_entry = (cd_entry*)((char*)tab_entries[t] + tab_entries[t]->cd_offset);
				for(int i = 0; i < tab_entries[t]->num_columns; i++, col_entry++){
					//printf("[parseWhereClause] comparing - %s %s\n", toLower(col_entry->col_name), toLower(cur->tok_string));
					if(strcmp(toLower(col_entry->col_name),toLower(cur->tok_string))==0){
						contains=true;
						conditionList[conditionNo] = *parseCondition(cur->next, col_entry->col_type);
						conditionList[conditionNo].colNo=i;
						conditionList[conditionNo].tableNo=t;
