...
bool** 				filterRows(char*** recordsList, tpd_entry **tab_entries, int tableCount, condition* conditionList, int conditionCount, int* rowCount, int rowLen);
...
struct row_obj{
	...
	bool lessThan(row_obj& other){
		if(dataType==T_INT){
			printf("%d-%d\n", bin2int(rowData+offset+1), bin2int(other.rowData+other.offset+1));
